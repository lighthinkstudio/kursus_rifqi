			...
			<div class="card-body">
				<form action="{{ route('admin.update_user', $user->id) }}" method="POST" class="form-horizontal" enctype="multipart/form-data">
					...
					<div class="form-group row">
						...
						<div class="col-md-9">
							<div id="imagePreview">
								<img id="oldImage" src="@if(!empty($user->foto)) {{ asset('storage/assets/uploads/images/' . $user->foto) }} @endif" alt="Image">
							</div>
						</div>
					</div>
					<div class="form-group row">
					...
					<div class="form-group row">
						...
						<div class="col-md-9">
							<input type="text" name="nama" class="form-control @error('nama') is-invalid @enderror" value="{{ $user->nama }}" placeholder="Nama lengkap">
							@error('nama')
							<em class="text-danger">{{ $message }}</em>
							@enderror
						</div>
